...
def remainingTime(current_step, total_timesteps = 20000, step_length = 0.5 ):
    total_secs = total_timesteps * 0.5 #Step length is in seconds.
    passed_secs = current_step * 0.5

    passed_hours = passed_secs / (60 ** 2)
    total_hours = total_secs / (60 ** 2)

    return total_hours - passed_hours
...
if __name__=="__main__":
    ...
    for i in range(save_intervals):
        ...
        rospy.loginfo("Learning is {} % complete ({} hours left)".format(i/save_intervals * 100, remainingTime(i/save_intervals * timesteps, timesteps, 0.5) ))
        rospy.loginfo("Saving partially trained model to {}.pkl".format(args.o))
        model.save(args.o)
